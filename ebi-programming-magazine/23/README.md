...
- OnOwnershipRequestですがこれはSetOwnerを呼んだプレイヤーとオーナーで呼ばれます。trueを返すことによりオーナーが切り替わるというもので、オーバーライドしない状態で常にtrueだと思っていたのですがそうではない様です。  
- この常にtrueを返すというコード自体は公式のページ内で記載されていたので問題は起こらないのではないかと思われます。ただしなぜこれでオーナーが分裂するという起こってはならない問題が解決するのか、またそれがデフォルトでないのかが全く分からないので、デメリットかないのかなども謎な感じです。
