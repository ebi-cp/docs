...
- Interactメソッドを実装しこのスクNoneリプトをオブジェクトに追加する事でボタンとして機能するようになりますがVR環境だとビームの伸びがVRとデスクトップで違うなどします。VRChatの立体物のインタラクトできるボタンは全てこれで作られていますが、uGUIを使ったボタンなどもありこちらはビームの伸びが安定しています。（ビデオプレイヤーなどのボタンです）
